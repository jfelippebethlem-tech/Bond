...
// SEC-01: token via header Authorization: Bearer (fora da query string).
function igUrl(path: string, extra = '') {
  return `${IG_BASE}${path}${extra ? '?' + extra : ''}`
}

function igFetch(url: string) {
  const token = process.env.FACEBOOK_PAGE_TOKEN // Instagram usa o mesmo token do Facebook
  return fetch(url, { headers: { Authorization: `Bearer ${token}` } })
}

export async function getInstagramAccountId() {
  if (!process.env.FACEBOOK_PAGE_TOKEN) return null
  const res = await igFetch(igUrl('/me', 'fields=instagram_business_account'))
...
export async function getInstagramPosts(igUserId: string, limit = 20) {
  ...
  const res = await igFetch(igUrl(`/${igUserId}/media`, `fields=${fields}&limit=${limit}`))
  if (!res.ok) return []
  const data = await res.json()
  return data.data ?? []
}
...
export async function getInstagramPostInsights(mediaId: string) {
  ...
  const res = await igFetch(igUrl(`/${mediaId}/insights`, `metric=${metrics}`))
  if (!res.ok) return null
  const data = await res.json()
...
export async function getInstagramComments(mediaId: string) {
  ...
  const res = await igFetch(igUrl(`/${mediaId}/comments`, 'fields=id,username,text,timestamp,replies{id,username,text,timestamp}&limit=100'))
  if (!res.ok) return []
  const data = await res.json()
  return data.data ?? []
}
...
export async function getInstagramBusinessDiscovery(meuIgId: string, username: string, limitPosts = 12) {
  ...
  const res = await igFetch(igUrl(`/${meuIgId}`, `fields=${sub}`))
  if (!res.ok) return null
  const data = await res.json()
